...
- **Flow:**
  ...
  5. Publish `ChallengeJoinedEvent` ở pha `AFTER_COMMIT`.
  6. Listener side effects (event-driven):
...
- **Flow:**
  ...
  4. Commit transaction.
  5. Publish `ChallengeQuitEvent` ở pha `AFTER_COMMIT`.
...
- **Flow:**
  ...
  5. Publish `SubmissionCreatedEvent` ở pha `AFTER_COMMIT`.
...
- **Flow:**
  ...
  4. Nếu status = `APPROVED`: publish `SubmissionApprovedEvent` ở pha `AFTER_COMMIT`.
...
2. **[AFTER_COMMIT Event Publish]**

- Publish `SubmissionApprovedEvent` sau commit transaction của submission.

3. **[Event Listeners - Async fan-out theo enterprise contract]**
...
> Việc thực thi listener theo cơ chế async chưa có annotation cấu hình tường minh (`@Async`/`@TransactionalEventListener`) trong mã hiện tại; contract vẫn yêu cầu async sau commit. `[Suy luận]`
...
Payload push realtime hiện tại: `id`, `type`, `title`, `message`, `metadata`, `read`, `createdAt`.
